...
#define NUM_BUFFER_BYTES    SETTINGS_ARRAY_SIZE+1
//endmarcus
...
void CreateSignalArray(int *settings){
    ...
    int currVal;
    int currByte;
    int buff[NUM_BUFFER_BYTES];
    buff[0] = 50 ; //this is the first byte we'll send (i.e. the flag byte)
    
    //fill up the rest of the buffer
    for (int m = 1; m<NUM_BUFFER_BYTES ; m++){
        buff[m] = settings[m-1];
    }
    
    for(int i =0; i <  NUM_BUFFER_BYTES; i++){ //loops through each byte
        currByte = buff[i];
        for(int k=0 ; k<TOTAL_BITS_PER_BYTE ; k++){ //loops through all bits of the byte
            ...
            else if(k == 0) currVal = 0; //start bit
            else if(k == TOTAL_BITS_PER_BYTE-1) currVal = 1;//stop bit,it gets inverted
    ...
    int importantBitsMultiplied = TOTAL_ARR_SIZE - (NUM_BUFFER_BYTES*TOTAL_BITS_PER_BYTE*BUFFER_MULT);
    
    for(int q = importantBitsMultiplied+1; q < TOTAL_ARR_SIZE; q++){
        bigBuffer[q] =  IDLE_HIGH;
    }
