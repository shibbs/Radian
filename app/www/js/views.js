...
$(document).ready(function() {
    ...
    window.views.TimeLapseDegreesView = window.views.BaseView.extend({
        ...
        render: function() {
            ...
            this.$el.empty().append(this.template(this.model.getTemplateJSON()));
			var statsView = new window.views.StatsBoxView({model: this.model});
		    ...
		    this.$('#picker').scroller({
		        ...
		        display: 'inline',
		        mode: 'scroller',
		        wheels: [{'Degrees': create_slot(0, 360, 5)}, {'Direction': { CW: 'CW', CCW: 'CCW' }}],
		        height: 35,
				rows: 3,
				onChange: function(valueText, instance) { window.app.set('isClockwise', instance.values[1]=='CW');
	...
	window.views.timeLapseView = new window.views.TimeLapseView({model: window.app});
	window.views.timeLapsePresetsView = new window.views.TimeLapsePresetsView();
	window.views.timeLapseDegreesView = new window.views.TimeLapseDegreesView({model: window.app});
	window.views.timeLapseTotalTimeView = new window.views.TimeLapseTotalTimeView({model: window.app});
	window.views.timeLapseIntervalView = new window.views.TimeLapseIntervalView({model: window.app});
